...
검증 데이터 비율 : 0.1
Optimizer : Adam
학습률 스케줄러 : ReduceLROnPlateau (검증 손실 기준)
Warm up : -
Data Augmentation : SpecAugment (훈련 데이터에만 적용)
모델 저장 : 검증 손실이 가장 낮은 모델을 'models/best_model.pth'에 저장

현재 사용 pth : Epoch 3 완료 | 훈련 Loss: 0.0183 | 검증 Loss: 0.5590 | 검증 정확도: 0.8012
--------------
'''
...
class TripletSpectrogramDataset(Dataset):
    def __init__(self, data_dir, song_list, instruments, data_map, transform=None):
    ...
    def __getitem__(self, index):
        ...
        anchor = torch.load(anchor_path, weights_only=True)
        positive = torch.load(positive_path, weights_only=True)
        negative = torch.load(negative_path, weights_only=True)

        if self.transform:
            anchor = self.transform(anchor)
            positive = self.transform(positive)
            # 네거티브 샘플은 증강하지 않음 (필요 시 주석 해제)
...
def main():
    ...
    data_map = defaultdict(nested_defaultdict)
    all_songs = set()
    for f in all_files:
    ...
    # --- val은 증강하지 않음 ---
    train_dataset = TripletSpectrogramDataset(args.data_dir, train_songs, instruments, data_map, transform=spec_augment)
    val_dataset = TripletSpectrogramDataset(args.data_dir, val_songs, instruments, data_map, transform=None)
